  ...
  brushMenu(e) {
    ...
    if (!CanvasApp.brushMenuStatus) {
      ...
      document
        ...
        .forEach((elem) => (elem.style.backgroundColor = CanvasApp.brushColor));
    } else {
      document.getElementById("brushes").style.display = "none";
      CanvasApp.brushMenuStatus = false;
    }
...
CanvasApp.canvas.addEventListener('mousemove', (e) => {
    cursor.style.top =( e.pageY) + 'px';
    cursor.style.left = (e.pageX) + 'px';
    if(e.pageY  <= 5)cursor.style.display = 'none'
...
document
  ...
  .addEventListener("click", CanvasApp.colorMenu);
document
  .querySelector(".brush-btn")
  .addEventListener("click", CanvasApp.brushMenu);

document.querySelectorAll(".color").forEach((elem) => {
  elem.addEventListener("click", (e) => {
    e.preventDefault();
    CanvasApp.brushColor = e.target.title;
    document.querySelector('.cursor').style.backgroundColor = e.target.title;
...
document.querySelectorAll(".brush").forEach((elem) => {
  elem.addEventListener("click", (e) => {
    ...
    document.querySelector('.cursor').style.width = `${Number(elem.classList[1].slice(4))*2}px`;
    document.querySelector('.cursor').style.height = `${Number(elem.classList[1].slice(4))*2}px`;
    document.getElementById("brushes").style.display = "none";
    CanvasApp.brushMenuStatus = false;
  });
});
